...
echo $IPADDRESS
sed -e "s/COMPONENT/${COMPONENT}/" -e  "s/IPADDRESS/$IPADDRESS/" robot/record.json > \tmp\record.json
aws route53 change-resource-record-sets --hosted-zone-id $HOSTEDZONEID --change-batch file:\tmp\record.json | jq
