#!/bin/bash
# cRT is same as catalogue - Node JS . so copy and change componenet

echo "cart automation script"

# set -e # exist the prog if any error ( disabled with # as comment for this prog)

COMPONENT=cart # to remove repetion of the name frontend. and also helps not to hardcode the filename
APPUSER=roboshop
